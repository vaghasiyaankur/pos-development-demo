                                ...
                                <div class="addcombo-card d-flex flex-wrap">
                                    <div class="addcombo-single-card mb-3 position-relative" v-for="product in allProducts" :key="product">
                                        <div class="card overflow-hidden" :class="{active : selproduct.includes(product.id)}" style="cursor:pointer" @click="addProduct(product.id)">
                                            <img v-if="selproduct.includes(product.id)" src="/assets/images/check.png" alt="editcombo" class="position-absolute text-success"  style="top: -1px; right: -1px; z-index:99;">
                                            <div class="card-body p-0">
                                                <div class="body-content position-relative">
                                                    <img :src="'/storage/'+product.image" class="combo-img" :id="'combo_image_'+product.id" alt="editcombo">
                                                    <p class="mb-0 fs-10 px-lg-2 px-1 pt-1 color-gray fw-bold line--clamp" :id="'combo_name_'+product.id">{{ product.name }}</p>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div class="row">
                                    <div class="col-7 pe-0">
                                        <div class="comboname-input">
                                            ...
                                            <input type="hidden" v-model="combo.id">
                                    ...
                                    <div class="form-group">
                                        <div class="fileUpload">
                                            <div v-if="image_url" class="fileUpload_preview">
                                                <img :src="image_url" class="img_fileUpload_preview" alt="editcombo">
                                            </div>
                                ...
                                <div class="addcombo-card d-flex flex-wrap">
                                    <div class="addcombo-single-card" v-for="item in selectedProduct" :key="item">
                                        <div class="card active overflow-hidden">
                                            <img  src="/assets/images/check.png" class="position-absolute text-success" alt="editcombo" style="top: -1px; right: -1px; z-index:99;">
                                            <div class="card-body p-0">
                                                <div class="body-content position-relative">
                                                    <img :src="item.image" class="combo-img" alt="editcombo">
                                                    <div class="px-lg-2 px-1 py-1">
            ...
            <div class="footer-main bg-white" style="position:fixed; bottom: 0; width: 100%;">
                <div class="footer-link d-flex align-items-center justify-content-between">
                    ...
                    <div class="add-catagory-btn">
                        ...
                        <button class="btn btn-success fs-16 btn-custom text-white" @click="EditCombo()">Edit Combo</button>
                    </div>
                </div>
            </div>
        </footer>
...
export default {
    ...
    methods:{
        ...
        getSingleCombo(){
            axios.get(`/api/getSingleCombo/`+this.$route.params.slug)
                .then(res => {
                    this.combo.id = res.data.combo.id;
                    this.combo.name = res.data.combo.name;
                    this.combo.price = res.data.combo.price.toFixed(2);
                    this.combo.foodType = res.data.combo.food_type;
                    this.combo.status = res.data.combo.status;
                    this.image_url = `/storage/`+res.data.combo.image;
                    this.combo.image = res.data.combo.image;
                    this.image_text = 'Edit Picture';

                    res.data.combo.combo_products.forEach(data => {
                        var product = {
                            id:data.product.id,
                            image:`/storage/`+data.product.image,
                            name:data.product.name
                        }
                        this.selproduct.push(data.product.id);
                        this.selectedProduct.push(product);
                    });
                    this.$emit('removeLoader')
                }).catch(err => {
...
<style scoped>
    ...
    /******* File upload Css End *********/
    .addcombo-main .addcombo-inner .addcombo-left .addcombo-card-main::-webkit-scrollbar-thumb {
        background: #8b8a8a;
        border-radius: 10px;
    }
    .addcombo-main .addcombo-inner .addcombo-left .addcombo-card-main::-webkit-scrollbar {
        width: 3px;
        background: transparent;
    }
    .addcombo-main .addcombo-inner .addcombo-right .addcombo-card-main::-webkit-scrollbar-thumb {
        background: #8b8a8a;
        border-radius: 10px;
    }
    .addcombo-main .addcombo-inner .addcombo-right .addcombo-card-main::-webkit-scrollbar{
        width: 3px;
        background: transparent;
    }
